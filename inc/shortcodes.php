...
function display_manager_roster_shortcode() {
    ...
    <div id="trade-block-modal" class="fantasy-modal fa-modal-hidden">
        <div class="fa-modal-content" style="max-width: 450px;">
            <span class="fa-modal-close"></span>
            <h3>Trade Block Settings</h3>
            <div id="trade-block-modal-message"></div>
            <form id="trade-block-form">
                <input type="hidden" id="tb-modal-playerid" name="player_id" value="">
                
                <p><strong>Player:</strong> <span id="tb-player-name-display" style="font-weight:bold; color:#0073aa;"></span></p>

                <div style="margin-bottom: 15px;">
                    <label>
                        <input type="checkbox" id="tb-on-block" name="on_block" value="1"> 
                        <strong>List on Trade Block</strong>
                    </label>
                </div>

                <div style="margin-bottom: 15px;">
                    <label for="tb-notes"><strong>Trade Notes / Team Needs:</strong></label><br>
                    <textarea id="tb-notes" name="notes" style="width: 100%;" rows="3" placeholder="e.g., Looking for a young Starting Pitcher..."></textarea>
                </div>

                <hr>
                <button type="submit" id="tb-submit-btn" class="button button-primary">Save Settings</button>
                <button type="button" class="button button-secondary fa-modal-cancel">Cancel</button>
            </form>
        </div>
    </div>
    ...
    <style>
    #restructure-modal.fa-modal-hidden, #extension-modal.fa-modal-hidden, #trade-block-modal.fa-modal-hidden { display: none !important; }
...
/**
 * Helper to render the actual trade form HTML.
 * Used by both the standalone page and the modal.
 */
function fod_render_trade_proposal_form_html($is_modal = false) {
    ...
    <form id="trade-proposal-form" class="trade-proposal-form-class" method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
        ...
        <p>
            <button type="submit" id="propose-trade-submit" disabled>Propose Trade</button>
            <?php if ($is_modal): ?>
                <button type="button" class="button button-secondary fa-modal-cancel">Cancel</button>
            <?php endif; ?>
        </p>
...
function display_trade_proposal_form_shortcode() {
    ...
    echo fod_render_trade_proposal_form_html(false);
...
/* ------------------------------------------------------------------------
   [league_trade_block] — View players listed for trade league-wide
------------------------------------------------------------------------ */
function display_league_trade_block_shortcode() {
    if ( ! is_user_logged_in() ) { return '<p>Please log in to view the trade block.</p>'; }
    if ( ! function_exists('get_field') ) { return '<p>Error: ACF not active.</p>'; }

    global $wpdb;
    $current_user_id = get_current_user_id();

    // 1. Get All Unique Leagues
    $all_leagues = $wpdb->get_col( 
        "SELECT DISTINCT meta_value FROM {$wpdb->postmeta} 
         WHERE meta_key = 'league_id' AND meta_value != '' 
         ORDER BY meta_value ASC" 
    ...
    // 3. Query for players on trade block in this league
    // Using a random order or a timestamp in meta_query isn't ideal for performance, 
    // so we use 'cache_results' => false and 'update_post_meta_cache' => false.
    $args = [
        'post_type'              => 'playerdata',
        'posts_per_page'         => -1,
        'cache_results'          => false, 
    ...
    <div class="trade-block-wrapper">
        ...
        <h2>League Trade Block (<?php echo esc_html($selected_league_id); ?>)</h2>

        <?php if ( ! $query->have_posts() ) : ?>
            <p>No players are currently listed on the trade block for this league.</p>
        <?php else : 
            $trade_page_id = get_field('trade_page', 'option');
            $trade_url_base = $trade_page_id ? get_permalink($trade_page_id) : '#';
            
            $players_by_team = [];
            while ( $query->have_posts() ) {
                $query->the_post();
                $pid = get_the_ID();
                $team = get_post_meta($pid, 'fantasy_team_id', true) ?: 'Unknown';
                
                // Get compact contract info (Next 3 years)
                $contract_parts = [];
                foreach(range(2026, 2028) as $yr) {
                    $val = get_post_meta($pid, 'contract_' . $yr, true);
                    if ($val) {
                        if (is_numeric($val)) {
                            $fmt = ($val >= 1000000) ? round($val/1000000, 1) . 'M' : round($val/1000) . 'K';
                            $contract_parts[] = " '" . substr($yr, -2) . ": $" . $fmt;
                        } else {
                            $contract_parts[] = " '" . substr($yr, -2) . ": " . $val;
                        }
                    }
                }
                ...
                $players_by_team[$team][] = [
                    ...
                    'notes' => get_field('trade_block_notes', $pid),
                    'mlb' => get_post_meta($pid, 'mlb_team', true),
                    'contract' => implode(',', $contract_parts),
                    'on_block' => get_post_meta($pid, 'on_trade_block', true)
                ];
            }
            wp_reset_postdata();

            foreach ( $players_by_team as $team_id => $players ) : 
                // Find the manager for this team once per team section
                $target_manager_id = fod_get_team_manager_id($selected_league_id, $team_id);
            ?>
                <div class="trade-block-team-section" style="margin-bottom: 30px;">
                    <h3 style="background: #f0f0f1; padding: 15px 20px; border-left: 4px solid var(--fod-orange-accent);">
                        <?php echo esc_html($team_id); ?>
                    </h3>
                    <table class="fantasy-table-base">
                        ...
                        <tbody>
                            <?php foreach ($players as $p) : 
                                $propose_url = add_query_arg([
                                    'trade_league' => $selected_league_id,
                                    'target_manager' => $target_manager_id,
                                    'requested_player' => $p['id']
                                ], $trade_url_base);
                            ?>
                                <tr>
                                    <td><strong><?php echo esc_html($p['name']); ?></strong></td>
                                    <td><?php echo esc_html($p['pos']); ?></td>
                                    <td><?php echo esc_html($p['mlb']); ?></td>
                                    <td style="font-size: 0.9em; font-family: monospace;"><?php echo esc_html($p['contract'] ?: '–'); ?></td>
                                    <td><em style="color: #666;"><?php echo esc_html($p['notes']); ?></em></td>
                                    <td>
                                        <?php if ( $target_manager_id && $target_manager_id != $current_user_id ) : ?>
                                            <button type="button" class="button button-small open-trade-proposal-modal" 
                                                data-league="<?php echo esc_attr($selected_league_id); ?>" 
                                                data-manager="<?php echo esc_attr($target_manager_id); ?>" 
                                                data-player="<?php echo esc_attr($p['id']); ?>">
                                                Propose Trade
                                            </button>
                                        <?php elseif ($target_manager_id == $current_user_id) : ?>
                                            <button type="button" class="button button-small open-trade-block-modal" 
                                                data-playerid="<?php echo esc_attr($p['id']); ?>" 
                                                data-playername="<?php echo esc_attr($p['name']); ?>" 
                                                data-onblock="<?php echo ($p['on_block'] === '1' ? 1 : 0); ?>" 
                                                data-notes="<?php echo esc_attr($p['notes']); ?>"
                                                style="background-color: #673ab7; color: white; border-color: #512da8;">
                                                Edit Listing
                                            </button>
                                            <button type="button" class="button button-small quick-remove-trade-block" 
                                                data-playerid="<?php echo esc_attr($p['id']); ?>" 
                                                style="background-color: #d9534f; color: white; border-color: #b52b27;">
                                                Remove
                                            </button>
                                        <?php else : ?>
                                            <span style="color: #999; font-size: 0.8em;">Manager N/A (Searching: <?php echo esc_html($selected_league_id . ' - ' . $team_id); ?>)</span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
    ...
    <!-- Trade Block Settings Modal (Shared with Roster Page) -->
    <div id="trade-block-modal" class="fantasy-modal fa-modal-hidden">
        <div class="fa-modal-content" style="max-width: 450px;">
            <span class="fa-modal-close"></span>
            <h3>Trade Block Settings</h3>
            ...
            <form id="trade-block-form">
                ...
                <p><strong>Player:</strong> <span id="tb-player-name-display" style="font-weight:bold; color:#0073aa;"></span></p>
                <div style="margin-bottom: 15px;">
                    <label><input type="checkbox" id="tb-on-block" name="on_block" value="1"> <strong>List on Trade Block</strong></label>
                </div>
                <div style="margin-bottom: 15px;">
                    <label for="tb-notes"><strong>Trade Notes / Team Needs:</strong></label><br>
                    <textarea id="tb-notes" name="notes" style="width: 100%;" rows="3" placeholder="e.g., Looking for a young Starting Pitcher..."></textarea>
    ...
    <div id="trade-proposal-modal" class="fantasy-modal fa-modal-hidden">
        <div class="fa-modal-content" style="max-width: 900px; max-height: 90vh; overflow-y: auto;">
            <span class="fa-modal-close"></span>
            <h3>Propose New Trade</h3>
            <div class="modal-form-wrapper" style="margin-top: 20px;">
                <?php echo fod_render_trade_proposal_form_html(true); ?>
            </div>
        </div>
    </div>

    <style>
        #trade-block-modal.fa-modal-hidden, #trade-proposal-modal.fa-modal-hidden { display: none !important; }
        .trade-block-wrapper .button-small { margin-right: 5px; }
    </style>
    <?php
    return ob_get_clean();
}
add_shortcode( 'league_trade_block', 'display_league_trade_block_shortcode' );
